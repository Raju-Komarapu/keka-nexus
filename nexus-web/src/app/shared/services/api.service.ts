...
export class ApiService {
    ...
    getFile(path: string, httpParams: HttpParams = new HttpParams()): Observable<HttpResponse<Blob>> {
        return this.httpClient.get(`${path}`, { observe: 'response', responseType: 'blob', params: this.removeNullValuesFromQueryParams(httpParams) }).pipe(mergeMap(response => this.mapResponse(response)), catchError(this.handleError));
    }

    getFileUsingPost(path: string, body: Object = {}, httpParams: HttpParams = new HttpParams()): Observable<HttpResponse<Blob>> {
        return this.httpClient.post(`${path}`, body, { observe: 'response', responseType: 'blob', headers: null, params: this.removeNullValuesFromQueryParams(httpParams) }).pipe(mergeMap(response => this.mapResponse(response)), catchError(this.handleError));
    }

    getPagedResult<T>(path: string, params: HttpParams = new HttpParams()): Observable<any> {
        return this.httpClient.get<T>(`${path}`, { headers: this.httpHeaders, params: this.removeNullValuesFromQueryParams(params) })
         .pipe(mergeMap(response => this.mapPagedResponse(response)), catchError(this.handleError));
    }

    getPagedResultUsingPost<T>(path: string, body: Object = {}, params: HttpParams = new HttpParams()): Observable<any> {
        return this.httpClient.post<T>(`${path}`, JSON.stringify(body), { headers: this.httpHeaders, params: this.removeNullValuesFromQueryParams(params) })
         .pipe(mergeMap(response => this.mapPagedResponse(response)), catchError(this.handleError));
    }

    getFileDetailsUsingPost(path: string, body: Object = {}, httpParams: HttpParams = new HttpParams()): Observable<any> {
        return this.httpClient.post(`${path}`, body, { headers: null, params: this.removeNullValuesFromQueryParams(httpParams) }).pipe(mergeMap(response => this.mapResponse(response)), catchError(this.handleError));
    }
